...
class TearsheetStatistics(AbstractStatistics):
    ...
    def update(self, timestamp, portfolio_handler):
        ...
        if self.benchmark is not None:
            self.equity_benchmark[timestamp] = self.price_handler.get_last_close(self.benchmark)
    ...
    def get_results(self):
        ...
        equity_s = equity_s.resample('D').last()        
        #print(equity_s)
        # Returns
    ...
    def _plot_txt_curve(self, stats, ax=None, **kwargs):
        ...
        if self.benchmark is not None:
            ...
            # https://www.mql5.com/en/articles/2358#c8
            # https://www.elitetrader.com/et/threads/equity-curve-r-squared.301176/
    ...
    def _plot_txt_trade(self, stats, ax=None, **kwargs):
        ...
        if 'positions' not in stats:
            ...
            #print(pos[pos["trade_pct"] > 0].shape[0])
    ...
    # os.path.expanduser() method in Python is used to expand an initial path component
    # ~( tilde symbol) or ~user in the given path to user’s home directory.
    def get_filename(self, filename=""):
        if filename == "":
            # now = datetime.utcnow()
            now = datetime.now()
            #filename = "tearsheet_" + now.strftime("%Y-%m-%d_%H%M%S") + ".png"
            filename = "tearsheet_" + now.strftime("%Y-%m-%d_%H%M%S") + ".pdf"
